...
const Auction = () => {
  ...
  const orders = async () => {
    try {
      const allorders = await axios.get(
        "https://energy-app-backend.vercel.app/api/auct/auction"
      );
  ...
  const bid = async (bidAmount, orderId, biddername, ownername , ownerid , maxunit ) => {
    ...
    if (biddername === ownername) {
      ...
      try {
        const res = await axios.post("https://energy-app-backend.vercel.app/api/auct/bid", data );
  ...
  const deleteitem = async (id) => {
    ...
    try {
      const dlt = await axios.delete(
        `https://energy-app-backend.vercel.app/api/auct/delete/${id}`,
