...
const Marketplace = () => {
  ...
  const orders = async () => {
    try {
      const allorders = await axios.get(
        "https://energy-app-backend.vercel.app/api/mak/marketord"
      );
      ...
      try {
        ...
        if (window.ethereum) {
          ...
          setTimeout(() => {
            ...
            axios.put(`https://energy-app-backend.vercel.app/api/mak/status/${id}`);
          }, 5000);
  ...
  const Tranaction = async (buyerWallet, sellerWallet, price, pincode) => {
    try {
      const res = axios.post("https://energy-app-backend.vercel.app/api/mak/marktra", {
        buyerWallet,
        sellerWallet,
        price,
        pincode,
  ...
  const deleteitem = async (id) => {
    try {
      const dlt = await axios.delete(
        `https://energy-app-backend.vercel.app/api/mak/delete/${id}`,
  ...
  const makePayment = async (data) => {
    ...
    try {
      ...
      const response = await fetch("https://energy-app-backend.vercel.app/api/mak/stripe/pay", {
        method: "POST",
        headers: headers,
      ...
      if (result.error) {
        ...
        //   const transactiondata = await axios.post("https://energy-app-backend.vercel.app/api/mak/stripe/Tra", datas)
