  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const statusResponse = await axios.put(`https://energy-app-backend.vercel.app/api/mak/status/${id}`);
  
        // Create a transaction using a POST request

        if(flag){
        const traResponse = await axios.post("https://energy-app-backend.vercel.app/api/mak/stripe/Tra", data);
           setflag(false);
      }
